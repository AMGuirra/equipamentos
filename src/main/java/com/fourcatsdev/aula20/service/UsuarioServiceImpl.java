...
public class UsuarioServiceImpl implements UsuarioService {
	...
	public void alterarDadosUsuario(Long id, Usuario dados) {
		...
		String novaSenha = dados.getPassword();
		if (novaSenha != null && !novaSenha.isEmpty() && !novaSenha.equals(usuario.getPassword())) {
			usuario.setPassword(criptografia.encode(novaSenha));
		}
	...
	public void alterarSenhaUsuario(Long id, String senha) {
		Usuario usuario = buscarUsuarioPorId(id);
		usuario.setPassword(criptografia.encode(senha));
		usuarioRepository.save(usuario);
	}
