...
public interface PedidoRepository extends JpaRepository<Pedido, Long> {
	...
	@Query(value = "select distinct usuario_id as id, CAST(data_pedido AS date) as data, usuario.nome, estado_pedido.nome as situacao \n"
			     + "  from pedido \n"
			     + "  join usuario \n"
		         + "    on usuario.id = pedido.usuario_id \n"
		         + "  join estado_pedido \n"
		         + "    on estado_pedido.id = pedido.estado_pedido_id \n"
		         + " where usuario.id = :idUsuario", nativeQuery = true)
	...
	@Query(value = "select distinct usuario_id as id, CAST(data_pedido AS date) as data, usuario.nome, estado_pedido.nome as situacao \n"
	         ...
	         + "    on usuario.id = pedido.usuario_id \n"
	         + "  join estado_pedido \n"
	         + "    on estado_pedido.id = pedido.estado_pedido_id", nativeQuery = true)
